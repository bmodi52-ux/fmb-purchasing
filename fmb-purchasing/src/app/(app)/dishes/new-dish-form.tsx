...
export function NewDishForm() {
  ...
  useEffect(() => {
    if (state.dishId) router.push(`/dishes/${state.dishId}`);
  }, [state.dishId, router]);
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-[2fr_1fr_1fr]">
          ...
          <label className="flex flex-col gap-1 text-sm">
            <span className="text-ink/70">
              Thaalis per batch <span className="text-ink/40">(if per batch)</span>
            </span>
            <input name="batch_thaalis" type="number" min="1" defaultValue="200" className="input" />
          </label>
        </div>
        <label className="flex flex-col gap-1 text-sm">
          <span className="text-ink/70">
